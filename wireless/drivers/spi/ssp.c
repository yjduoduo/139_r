...
void  ssp1_init(void)
{
    //P0.4,0.5,0.6 fun(00) 0 gpio
    //P0.7,0.8,0.9 fun(10) 3 fun
    PCONP 	|= (1 << 10);		    									/* 打开SSP1电源*/
    PINSEL0 &=~(1<<8); //p0.4
    PINSEL0 &=~(1<<9);//WR
    PINSEL0 &=~(1<<10);//p0.5
...
*********************************************************************************************************/
uint8 spi_falsh_sendbyte(uint8 tmp)
{
...
void M25P80_Write_En(void)
{
    ...
    tmp = M25P80_CMD1B_READ1B(RDSR) & (~0x7c);   // ~00011100  1110 0011
...
void M25P80_Section_Erase(uint32 addr)
{
    ...
    M25P80_Write_En();
    SPI_CS_LOW();
    spi_falsh_sendbyte(SECTOR_ERASER);
    spi_falsh_sendbyte(ad[0]);
    spi_falsh_sendbyte(ad[1]);
    spi_falsh_sendbyte(ad[2]);
    SPI_CS_HIGH();
    while(M25P80_Busy());
    M25P80_WP_En();
...
uint8 M25P80_Read_1Byte(uint32 addr )
{
    ...
    spi_falsh_sendbyte(READ);
    spi_falsh_sendbyte(ad[0]);
    spi_falsh_sendbyte(ad[1]);
    spi_falsh_sendbyte(ad[2]);
    data = spi_falsh_sendbyte(0xff);
    SPI_CS_HIGH();//高阻
    return data;
}
...
void M25P80_Read_Bytes(uint32 addr , uint8* re_buf_p , uint16 no)
{
    ...
    spi_falsh_sendbyte(ad[2]);
    for(td=0; td<no; td++)
        *re_buf_p++ = spi_falsh_sendbyte(0xff);
    SPI_CS_HIGH();
    while(M25P80_Busy());
}
...
* Return         : return data;
*******************************************************************************/
uint8 M25P80_ReadFast_1Byte(uint32 addr )
{
    uint8 ad[3] , data;
    ...
    spi_falsh_sendbyte(FAST_READ);
    spi_falsh_sendbyte(ad[0]);
    spi_falsh_sendbyte(ad[1]);
    spi_falsh_sendbyte(ad[2]);
    spi_falsh_sendbyte(0x00);
    data = spi_falsh_sendbyte(0xff);
    SPI_CS_HIGH();//高阻
    return data;
}
